...
\chapter{Geodesic Fields}
	...
	In presence of a pseudo-Riemannian metric is possible to present the geodesic in a metric sense i.e. as the curve extremizing the \emph{Energy Functional}\footnote{Remember that for arc-length parametrized curves the Energy functional coincide with the length functional.\cite[Lemma $1.4.2$ ]{Jost2005}}:
	\begin{definition}[Energy functional]
  	\begin{equation}\label{EnergyFunctional}
  		E: C^1\left(\left[a,b\right],Q\right) \rightarrow \Real \qquad
...
\end{definition} 	
	...
	\end{definition}
		...
		\begin{itemize}
			...
			\item $R^\mu_{i \alpha \j}$ is the components representations of the Riemann curvature tensor,
		\end{itemize}
...
\section{Geodesic Problem as a Mechanical Systems}\label{GeodesicMechanics}
	...
	\end{corollary}
	\begin{proof}
		Is a simple application of the correspondence seen in chapter \ref{MechanicsAsAField}.
	\end{proof}
	
	From this perspective is clear that the Energy Functional corresponds to the action of the geodesic field dynamics and equation \ref{GeodesicEquation} is nothing more than the motion equation according to the \emph{least action principle}.
...
\section{Peierls Bracket of the Geodesic field}
	The local coordinate expression for the Lagrangian density of the geodesic field results:
	\begin{equation}
...
\section{Algebraic quantization of the Geodesic Field}
	...
	Since equation\ref{GeodesicEquation} is highly non linear,  it is not the geodesic system that can actually be quantized but rather its linearization, the Jacobi field along a fixed geodesic $\gamma_0$.
		...
		\subsubsection{Classical Framework}
			The basic idea is that, chosen a geodesic $\gamma_0$, the kinematics configurations of the Jacobi fields are tangent fields along the fixed curves.
			...
			\paragraph{Dynamics}
				...
				According to equation \ref{Eq:NormallyHyperbolicRepresentation} this operator fall exactly in the \emph{normally hyperbolic} operator class hence is quantizable both by Peierls procedure and by initial data.
		...
		\subsubsection{Initial data Approach}
			...
			\paragraph{Symplectic Structure on the Phase Space}		
				...
				\\
				Which can be transferred on the solutions space:
				\begin{displaymath}
	...
	\subsection{Comparisons}
		The two procedures yields two different classical symplectic spaces: $(\Obs,\tau)$ and $(\Sol, \sigma)$.
