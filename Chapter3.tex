   		...
   		The concrete construction is achieved in three steps.
 			...
 			\subparagraph{PreObservables}
 				...
 				\begin{displaymath}
					...
					%Vecchia espressione:
					%	\Obs_0 \coloneqq \big\{ F_f: \Conf \rightarrow \Real ;  F_f(\phi)=( f, \phi) \forall \phi \in \Conf \:\vert 	\:  f \in \Gamma_0^\infty(E)	\big\}
				\end{displaymath}
...
%					Is defined the effect (eq: \label{EffectOperator}) of a Lagrangian density   as:
			...
			\subparagraph{Symplectic form on the Classical Observables}
				...
				% This condition is not a drawback, in fact it permit to transport naturally the bilinear operator $\tau$  from $\Gamma_0$ to the equivalence classes of $\Obs$:
