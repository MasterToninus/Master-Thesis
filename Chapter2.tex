...
In this chapter we will take advantage of the language developed above in order to  formalize precisely each step of the original Peierls' procedure\cite{Peierls1952} and thus establish the class of applicability of this algorithm. 
...
\fi
	...
	%At this level of abstraction, since no space-time structure has been specified, terms like stasis and motion must be taken with care .
...
%	 \end{observation}	
	...
	\end{definition}	
%	\begin{proposition} 
	%(Correzione CD)Quando si parla di distribuzioni, si tende a sottolineare che sono funzionali lineari sullo spazio delle test-funzioni. In realtà questo non basta, il funzionale deve essere anche continuo. E' questa la vera proprietà dominante e mi premeva sottolinearti di sottolinearla.
	%(COntestazione CD) A voler essere pignoli, il problema non è la linearità, ma la continuità rispetto alla topologia di D'(M). Le distribuzioni sono funzionali lineari e continui. Siamo sicuri che siano tutti continui e che non ci sia una drastica riduzione a chiedere una mappa in D'(M)?
	Notice that $\mathcal{O}_\Lagrangian [\phi] (f)$, as  a distribution, is necessarily continuous and linear in the test-functions entry  but not in the configurations entry.	
%	\end{proposition}	
...
%	\begin{notationfix}	% Correzione CD: nomenclatura non standard
...
%\newpage
	...
	\begin{remark}
		...
		\\
		The key is the definition of the Legendre Map $TL: TQ \rightarrow T^*Q$,  in the case that the Lagrangian $L$  is \emph{hyperegular}  ( \textit{i.e.} $TL$ is a diffeomorphism),it is possible to push-forward $L$ to give a proper Hamiltonian on $\Phase=T^*Q$. (see for example \cite{Abraham1978})
	\end{remark}	
...
%		\end{thesis}
	...
	\begin{definition}[Space of Classical Observables]
%	\begin{notationfix}
...
%	\end{observation}
	...
	\begin{definition}[Poisson Brackets]
		...
		\end{equation}
	\end{definition}
	%Nota di CD: andrebbe ricordata l'eguaglianza di Jacobi http://math.stackexchange.com/questions/557358/showing-jacobi-identity-for-poisson-bracket
	% prop 3.3.17 fomm
	\begin{proposition}
...
%	\begin{TAM}
		...
		\begin{itemize}
			...
			\item The same symplectic structure can be reproduced on the space $\Obs_\textrm{lin}$ and coincides with the reduction of the Poisson bracket to this subspace.
			\end{itemize}
%	\end{TAM}
...
%		\begin{observation}
			...
			%consiglio CD Se non hai in mente una referenza, credo che il libro "Analysis, tensor and manifolds" abbia quello che fa al caso tuo.
%		\end{observation}
		\\		
		...
		Remembering that every Lagrangian density define a continuous functional (Def. \ref{Def:LagrangianFunctionals})\ifToninus (the "Action")\fi.
	...
	\subsection{Extension to non-linear theories}\label{Section:NonLinearPeierls}
		...
		Anyway, it is possible to  recover a notation similar to Eq. \ref{PerturbedSolution} by working patchwise, under the choice of a particular coordinate representation.
		\\
