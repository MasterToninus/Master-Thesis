...
%	 \end{observation}	
	...
	\begin{definition}[Lagrangian Densities on the bundle $E$]\label{Def:LagrangianDensities}
		...
		\begin{displaymath}
			\gls{Lag}^r (E) \coloneqq \Gamma \left( \Hom\biggr(J^r E,\quad \bigwedge^m( T^*M)\biggr) \right) \cong \big\{f:\Gamma^\infty(J^r E) \rightarrow \Omega^m(M)  \big\}
		\end{displaymath}
...
%	\end{proposition}
	We have a \emph{Linear Lagrangian Functional} when $\Lagrangian$ is linear, this is not generally guaranteed by definition \ref{Def:LagrangianDensities} even if $E$ is a vector bundle. Has to be stressed that are not morphism between vector bundles but only between smooth bundles.
...
%	\end{observation}	
	...
	\subsubsection{Disturbance and Disturbed motion operator }
		...
		\\
		The support condition is %required
...
%		\end{observation}
		...
		\vspace{2mm}
		In conclusion, for any fixed $\phi\in \Sol$ and disturbance $\chi$, there exist two uniquely perturbed solutions:
		...
		\begin{displaymath}
			\gls{EffectOp} : C^1(\Conf,D'(M)) \rightarrow C^1(\Sol,D'(M))
		\end{displaymath}
...
%		\end{observation}
		...
		\end{displaymath}
		with the important difference that $G^\pm_{\tiny \gamma_0} $ are now the Green operators of the linearized %equation of motion (Correzione CD)
		equation of motion
