...
\fi	
	...
	\subsection{Geometric Mechanics of Classical Fields}
		...
		It is no secret that geometry is an intrinsic part of mechanics. For example the space of all admissible configurations of a mechanical system has the natural geometric structure of a manifold.
		Therefore the bet of geometric mechanics is that is possible to attribute to every mechanical system a  \emph{phase space} together with the implicit claim that it is possible to reconstruct from this space each physical quantity relevant to the mechanics. 
		...
		The reason is that from the symplectic structure descends in a simple way a Poisson structure which it lends itself to be modified in order to represent  the pre-quantum versions of the canonical commutation rules.
		...
		The “covariant functional formalism”, strongly advocated in the 1980’s by Crnkovi´c,Witten and Zuckerman [1–3] (see also [4]) who showed how to construct a symplectic structure starting from a "covariant presymplectic current density"\cite{Crnkovic1999}\cite{Khavkine2014}.
		This approach is
		This approach is slightly different from the construction "by initial data", a glimpse on such object can be found in example \ref{} where we have constructed a conserved current in order to prove that the symplect form $\Omega$ is independent from the choice of the cauchy surface.
