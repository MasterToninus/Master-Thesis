...
\subsection{Example: Geodesic field on FRW space-time.}
	...
	We recall that the Friedmann–Lemaître–Robertson–Walker spacetimes are homogeneous and isotropic solutions of Einstein’s equations whose line element is:
	\begin{equation}
	...
	\end{equation}
		...
		Accordingly with the three possible values of $k$, the assumptions of homogeneity and isotropy alone determine the spacetime metric up to the three discrete possibilities of spatial geometry.
		Namely the manifold
		Accordingly with the 
		the background is R×Σ, where Σ is locally homeomorphic to R3, S3, the 3-sphere or H3, the three-dimensional hyperboloid.
		...
		We want to construct the Peierls bracket for a simple base geodesic on the "flat" FRW solution:
		\begin{displaymath}
			ds^2 = -dt^2  a(t) \left( dx^2 + dy^2 + dz^2 \right)
		\end{displaymath}
		
		In the conformal coordinates we find that
		

		
		On this manifolds the geodesic equation are
		
		\cite{Wald1984}
		
		Consider now the most simple isotropic observer:
		\begin{displaymath}
			\gamma(t)^\mu = ( c(t),0,0,0)
		\end{displaymath}
		This is a geodesic iff:
		\begin{displaymath}
			c(t) = a + b t
		\end{displaymath}
		
		% \url{http://www.nada.kth.se/~annak/greens1d_odes.pdf}
		
		
		The Jacobi equation along this solution are:
