...
\fi
	...
	Let us visualize this construction in details.
		...
		\begin{displaymath}
			X(s) = a + b s
		\end{displaymath}
		therefore $\Sol \simeq \Real^2$ is a two dimesional linear subspace of the linear manifold:
		\begin{displaymath}
			\Gamma_0 = C_0^\infty (\Real) = \{ f: \Real \rightarrow \Real \; \vert \, \textrm{ \tiny smooth, compactly supported}\}
		\end{displaymath}
		Moreover the Green operators have an exact expression:
		\begin{displaymath}
			G^\pm \psi (s) = \int_{-\infty}^\infty \left(\psi(\xi) \theta\left( \pm\left(s-\xi\right) \right) \left( a_\pm + \xi b_\pm \right) \right) 
		\end{displaymath}
		correspondent to the Green functions of Eq. \ref{SimpleGreenFunction}.

		\begin{Warning}
			Forse ho sbagliato qualcos:\\ non mi risulta che $E \psi$ sia una soluzione, ovvero sia nella forma $(a +b s)$.
		\end{Warning}
